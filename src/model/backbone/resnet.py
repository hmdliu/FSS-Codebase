...
__all__ = ['get_resnet', 'resnet50', 'resnet101']

def get_resnet(arch, pretrained=True, resnet_kwargs={}):

    assert arch in ('resnet50', 'resnet101')

    if arch == 'resnet50':
        resnet = resnet50(pretrained=pretrained, **resnet_kwargs)
    else:
        resnet = resnet101(pretrained=pretrained, **resnet_kwargs)
    ...
    return nn.Sequential(layer0, layer1, layer2, layer3, layer4)
...
if __name__ == '__main__':
    ...
    # make model
    model = get_resnet('resnet50', pretrained=True, final_pool=True)
    print(model)
